...
public class ReservationService {
    ...
    public Reservation createReservation(User teacher, Long classroomId,
        ...
        // 受講可能人数（定員）のチェック：教室の定員を超えて設定できないようにする
        if (capacity == null || capacity <= 0) {
            throw new IllegalArgumentException("受講可能人数は1人以上で指定してください");
        }
        if (classroom.getCapacity() != null && capacity > classroom.getCapacity()) {
            throw new IllegalArgumentException(
                classroom.getName() + " の定員（" + classroom.getCapacity() + "人）を超える人数は設定できません");
        }
